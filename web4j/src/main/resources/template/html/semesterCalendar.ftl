...
[#else]
  [#if tag.label??]<label for="${tag.id}" class="title" style="font-weight:inherit">[#if tag.required]<em class="required">*</em>[/#if]${tag.label}:</label>[/#if]
  <input id="${tag.id}" class="calendar-text calendar-text-state-default" style="height:22px;" title="${tag.title}" type="text" value="[#if (tag.defaultValue)??]${tag.defaultValue.schoolYear}学年${tag.defaultValue.name}学期[#elseif tag.empty]全部学期[/#if]" readonly="true" ${tag.parameterString}/>
