...
(function($) {
    $.widget("ui.tagit", {
            ...
            this.input.keydown(function(e) {
                ...
                if (self._isInitKey(e.which)) {
                    ...
                    if ($(this).val().length >= self.options.minLength)
                        self._addTag($(this).val(), $(this).val());
                }
        ...
        _addSelect: function(label, value) {
            this.select.append('<option selected="selected" value="' + value + '">' + label + '</option>');
        }
        ,
        ...
        _addTag: function(label, value) {
            ...
            label = label.replace(/,+$/, "");
            label = label.trim();
            if (label == "" || this._exists(label))
                return false;

            var tag = "";
            tag = '<li class="tagit-choice">' + label + '<a class="tagit-close">x</a></li>';
            $(tag).insertBefore(this.input.parent());
            this.input.val("");
            //this.tagsArray.push(value);
            this.tagsArray.push(label);
            if (this.options.select)
            ...
            if (this.options.initialTags.length != 0) {
                for (var i in this.options.initialTags)
                    if (!this._exists(this.options.initialTags[i]))
                        this._addTag(this.options.initialTags[i], this.options.initialTags[i]);
            }
        }
        ,
