/**
    ...
    function displayTeacherList(teachSelectId,departId,refresh){
        ...
        if(teacherMap[departId]==null){
            ...
            teacherDao.getTeacherNamesByDepart(departId,true,setTeacherOptions);
        }
        if(refresh){
    ...
    function setTeacherOptions(data){
       ...
       dwr.util.addOptions(curTeacherSelectId,[{'id':'','name':'请选择..'}],'id','name');
       for(var i=0;i<data.length;i++){
           
           dwr.util.addOptions(curTeacherSelectId,[{'id':data[i][0],'name':data[i][1]+' ['+data[i][2]+']'}],'id','name');
