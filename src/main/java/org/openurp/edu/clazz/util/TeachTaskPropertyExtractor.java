  ...
  public Object getPropertyValue(Object target, String property) throws Exception {
    ...
    // 教材
      ...
      if (material != null) {
        ...
        OqlBuilder<CourseMaterial> queryCourse = OqlBuilder.from(CourseMaterial.class, "book");
        queryCourse.where("book.course = :course", clazz.getCourse());
        queryCourse.where("book.department = :department", clazz.getTeachDepart());
        queryCourse.where("book.semester = :semester", clazz.getSemester());
        CourseMaterial courseMaterial = entityDao.uniqueResult(queryCourse);
        if (courseMaterial != null) {
          for (Iterator<Textbook> iter = courseMaterial.getBooks().iterator(); iter.hasNext(); ) {
            Textbook book = iter.next();
            sb.append(MessageFormat.format("名称:{0},作者:{1},ISBN:{2},出版社:{3}", book.getName(),
    ...
    // 教室可容纳人数
      ...
      for (Session activity : clazz.getSchedule().getSessions()) {
