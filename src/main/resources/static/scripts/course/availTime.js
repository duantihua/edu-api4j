   var Weeks=["Mon","Tus","Wen","Ths","Fri","Sat","Sun"];
   var maxUnit=14;
	...
	function abbreviate(available) {
		...
		for (var i = 0; i < 7; i++) {
			...
			if (dayAvalible.indexOf('0')==-1) {
				...
				sb+="(";
				var morning = abbreviateUnit("morning", dayAvalible
						.substring(0, 5), 0);
				
				var afternoon = abbreviateUnit("afternoon", dayAvalible
						.substring(5, 10), 5);
				
				var evening = abbreviateUnit("evening", dayAvalible
						.substring(10, 14), 10);
				var hasOne=false;
				if("" !=morning ){
				   hasOne=true;
 ...
 colors["0"]="yellow";
 	...
 	function changeTimeMap(value) {
		for (var i = 0; i < value.length; i++) {
			var char = value.charAt(i);
			var td = document.getElementById(i.toString());
			var inp = document.getElementById('unit' + i.toString());
			inp.value = char;
			if(char == '0')
