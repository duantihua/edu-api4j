    		...
    		if(divs[i]==divID){
    		   ...
    		   if(divID=='courseListDIV'){
      		     	if(courseListFrame.location=="about:blank"){
	      		     	//dealthWithDIV(findIndex);
	      		        document.searchTaskFrom.submit();
      		     	}
      		   } else if (divID == 'teachclassDIV') {
	      		    if (teachclassFrame.location == "about:blank") {
	      		     	//dealthWithDIV(findIndex);
	      		        document.teachclassForm.submit();
	      		    }
      		   }
    		}
    		else{
...
//         var now = new Date(new Date().valueOf() - nowLength);
    ...
    function canElect(state,task){
        //检查选课开关
        if(!state.profile.isOpenElection){
          return  errors.notOpen;
        }
        // 检查评教
        if(state.profile.isCheckEvaluation&&!state.isEvaluated){
          return errors.needEvaluation;
        }
        //检查选课时间
        if(""!=checkDate(state)){
          return errors.noSuitableDate;
        }
        //检查学分
        ...
        // 检查HSK
        if(0!=electState.HskLevel&&0!=task.electInfo.HskLevel){
            if(electState.HskLevel<task.electInfo.HskLevel){
     ...
     */
       ...
       if(""!=scope.startNo&&""!=scope.endNo){
         ...
         if(""!=scope.adminClassIds){
             ...
             if(scope.departIds==",1,"){
                 if(""!=scope.grades){
                    if(scope.grades.indexOf(","+state.std.state.grade+",")==-1){
                      return errors.notInScope;
                    }
                 }
                 ...
                 //alert(scope.grades)
                 if(""!=scope.grades&&scope.grades.indexOf(","+state.std.state.grade+",")==-1){
                    return errors.notInScope;
                 }
                 ...
                 //alert(scope.majorIds)
                 if(""!=scope.majorIds&&scope.majorIds.indexOf(","+state.std.majorId+",")==-1){
                    return errors.notInScope;
                 }
                 //alert(scope.directionIds)
                 if(""!=scope.directionIds&&scope.directionIds.indexOf(","+state.std.state.directionId+",")==-1){
                    return errors.notInScope;
                 }
                 return "";                     
             }
     ...
     function removeTask(){
        ...
        table.tBodies[0].removeChild(selectInput.parentNode.parentNode);
	    for(var k=0;k<unitsPerWeek;k++){
			var td = document.getElementById("TD"+k);
    ...
    function getTaskList(){        
        ...
        document.getElementById("viewTD2").click();
    }
    ...
    function cancelTask(stdCode){
        ...
        if(taskId!==""&& confirm("确定要退掉这门课吗？")){
           if(!electState.profile.isOpenElection){
              alert(errors.notOpen);
              return  errors.notOpen;
           }
           if(electState.profile.isCheckEvaluation&&!electState.isEvaluated){
              alert(errors.needEvaluatio);
              return errors.needEvaluation;
           }
           if(""!=checkDate(electState)){
              alert(errors.noSuitableDate);
              return errors.noSuitableDate;
           }
